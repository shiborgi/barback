...
Run it on the Barback NAT network *without* `--dns` so `/etc/resolv.conf` contains only the container runtime's upstream resolvers. Mount the reconciler-generated `/records` directory read-only. The reconciler runs the resolver as UID 0 so it can read owner-only state and bind port 53; it grants only `CAP_NET_BIND_SERVICE`. It follows the atomically swapped `/records/current` bundle containing `db.barback.internal` and `lease.json`. Set `BARBACK_STACK_ID` and `BARBACK_DNS_GENERATION` to the active resolver identity.
